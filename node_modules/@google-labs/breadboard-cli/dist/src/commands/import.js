...
export const importGraph = async (url, options) => {
    ...
    if (apiPath == undefined && outputPath == undefined) {
        console.error("You must specify either -a (an API) or a directory to output all of the APIs to.");
        return;
    }
    let openAPIData = "";
    let json;
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Unable to parse OpenAPI spec from ${url}. It's not a valid JSON or YAML file.`);
        }
    }
    const openAPIBoard = await Board.fromGraphDescriptor(OpenAPI);
    const boards = await openAPIBoard.runOnce({ json }, { kits: [asRuntimeKit(CoreKit), asRuntimeKit(templates)] });
...
const outputBoard = async (board, apiPath, outputPath) => {
    const boardJSON = JSON.stringify(board);
    const boardName = apiPath;
    const boardPath = path.join(path.resolve(process.cwd(), outputPath), `${boardName}.json`);
