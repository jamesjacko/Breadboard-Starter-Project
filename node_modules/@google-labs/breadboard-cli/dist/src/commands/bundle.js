...
export async function bundle(board, flags) {
    let breadboardWebPublic;
    if (import.meta.resolve) {
        const publicPath = await import.meta.resolve("@google-labs/breadboard-web/public");
    ...
    if (Array.isArray(bundle)) {
        ...
        await Promise.all([
            ...
            // The board.
            fs.copyFile(board, path.join(flags.output, "graphs", "bundled-board.json")),
            // Styles.
            fs.cp(path.join(breadboardWebPublic, "styles"), path.join(flags.output, "styles"), { recursive: true }),
            // Third Party.
            fs.cp(path.join(breadboardWebPublic, "third_party"), path.join(flags.output, "third_party"), { recursive: true }),
            // Images.
            fs.cp(path.join(breadboardWebPublic, "images"), path.join(flags.output, "images"), { recursive: true }),
            // Boards.
            fs.cp(path.join(breadboardWebPublic, "graphs"), path.join(flags.output, "graphs"), { recursive: true }),
        ]);
    }
    else {
