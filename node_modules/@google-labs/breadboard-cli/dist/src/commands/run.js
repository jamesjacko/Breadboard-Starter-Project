 ...
 */
import { BoardRunner, asRuntimeKit, } from "@google-labs/breadboard";
...
async function runBoard(board, inputs, kitDeclarations, verbose, pipedInput = false) {
    const kits = [];
    ...
    for await (const stop of board.run({
        ...
        if (stop.type === "input") {
            ...
            const schema = nodeInputs.schema;
            /*
                    ...
                    for (const [name, property] of properties) {
                        if (name in newInputs == false) {
                            ...
                            if (required.indexOf(name) >= 0) {
                                ...
                                answer = await rl.question(`(${name}) ${property.description}:`);
                            }
                            else {
...
const loadInputFile = async (filePath) => {
    const fileUrl = pathToFileURL(filePath);
    return JSON.parse(await readFile(fileUrl.pathname, { encoding: "utf-8" }));
};
export const run = async (file, options) => {
    const kitDeclarations = options.kit;
