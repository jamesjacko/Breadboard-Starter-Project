...
export const watch = (file, options) => {
    let { controller, onChange, onRename } = options;
    onChange = onChange ?? (() => "");
    onRename =
        onRename ??
            ((filename) => console.error(`File ${filename} has been renamed. We can't manage this yet. Sorry!`));
    controller = controller ?? new AbortController();
    fsWatch(file, { signal: controller.signal, recursive: true }, async (eventType, filename) => {
...
export const loadBoard = async (file, options) => {
    const loaderType = extname(file).slice(1);
    ...
    const outputRoot = "output" in options ? options["output"] : path.dirname(file);
...
const showError = (e, path) => {
    const error = e;
    console.error(`Failed to load board at "${path}": ${error.message}`);
};
export const loadBoards = async (path, options) => {
    const fileStat = await stat(path);
    const fileUrl = pathToFileURL(path);
    if (fileStat && fileStat.isFile() && path.endsWith(".json")) {
        try {
            const data = await readFile(path, { encoding: "utf-8" });
            const board = JSON.parse(data); // assume conversion would fail if it wasn't a graph descriptor.
            return [
                {
                    edges: board.edges ?? [],
                    nodes: board.nodes ?? [],
...
async function loadBoardsFromDirectory(fileUrl, path, options) {
    ...
    for await (const dirent of dir) {
        ...
        if (dirent.isDirectory()) {
            const baseFolder = fileUrl.pathname.endsWith("/")
                ? fileUrl.pathname
                : `${fileUrl.pathname}/`;
            const boardsInDir = await loadBoardsFromDirectory(new URL(dirent.name, pathToFileURL(baseFolder)), join(path, dirent.name), options);
