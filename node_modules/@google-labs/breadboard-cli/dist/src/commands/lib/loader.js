...
export class Loader {
    async makeFromSource(filename, source, options) {
        const board = await this.loadBoardFromSource(filename, source, options);
        const boardJson = JSON.stringify(board, null, 2);
        return { boardJson, board };
    }
    async makeFromFile(filePath) {
        const board = await this.loadBoardFromModule(resolve(process.cwd(), filePath));
        const boardJson = JSON.stringify(board, null, 2);
        return { boardJson, board };
    }
    async loadBoardFromModule(file) {
        ...
        if (board instanceof Board == false &&
            ...
            throw new Error(`Board ${file} does not have a default export of type Board, Lambda or something that looks like a board.`);
        return board;
    }
    /*
  ...
  */
    async loadBoardFromSource(filename, source, options) {
        const tmpDir = options?.output ?? process.cwd();
        const randomName = Buffer.from(crypto.getRandomValues(new Uint32Array(16))).toString("hex");
        const filePath = join(tmpDir, `~${basename(filename, "ts")}${randomName}tmp.mjs`);
        let tmpFileStat;
        try {
        ...
        if (tmpFileStat && tmpFileStat.isFile()) {
            ...
            throw new Error(`The temporary file ${filePath} already exists. We can't write to it.`);
        }
        ...
        if (tmpFileStat && tmpFileStat.isDirectory() == false) {
            ...
            throw new Error(`The file ${filePath} is a directory. We can't write to it.`);
        }
