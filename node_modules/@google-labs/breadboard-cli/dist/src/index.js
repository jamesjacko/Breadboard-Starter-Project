...
program
    ...
    .option("-o, --output <path>", "Sets the output directory of the compiled graph (current directory by default.)", path.join(process.cwd(), `build-${new Date().toISOString().replaceAll(/\W/gim, "-")}`))
    .action(bundle);
program
    .command("import [url]")
    .description("Imports an OpenAPI spec and creates a dedicated board.")
    .option("-a, --api <path>", "Which API path / name will be exported. If this is left out, -o is required.")
    .option("-o, --output <path>", "The path where the boards will be output the board(s) to (current directory by default.)", process.cwd())
    .action(importGraph);
program
    .command("make [file]")
    .description("Make a graph from a javascript file. Note:all the imports have to be resolvable from the current directory.")
    .option("-o, --output <path>", "If compiling a graph in Typescript (.ts), you can control the output directory of the compiled graph (current directory by default.)", process.cwd())
    .option("-w, --watch", "Watch the file for changes.")
    .action(makeGraph);
program
...
program
    ...
    .option("--input-file <input>", "The path to a JSON file that represents the input to the graph.")
